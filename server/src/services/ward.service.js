...
export async function getWardLeaderboard() {
    ...
    return {
        wards: wards.map((w, i) => ({ ...w, rank: i + 1 })),
    };
}
...
export async function recomputeWardStats(wardId) {
    ...
    const totalComplaints30d = await Complaint.countDocuments({
        wardId,
        createdAt: { $gte: since30d },
    });
    const resolutionRate =
        totalComplaints30d > 0 ? Math.round((totalResolved / totalComplaints30d) * 100) : 100;

    const avgResolutionHours = resolutionAgg[0] ? Math.round(resolutionAgg[0].avg) : 0;

    const backlogPenalty = Math.min(totalOpen * 2, 30);
    const speedPenalty = avgResolutionHours > 72 ? 15 : 0;
